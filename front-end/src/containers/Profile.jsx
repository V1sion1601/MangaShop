...
import { updatepassword, updateprofile, getorderbyId } from "../utils/dummyData";
...
const Profile = () => {
  ...
  const submitEdit = async(e) => {
    ...
    if (!isValid) return;
    const res = await updateprofile(user.ID,editName,editAddress.replace("/","."),editPhone,user.ID_account);
    ...
    <div className="h-screen">
      <div className="h-fit flex flex-row items-start justify-start">
        ...
        <div className="w-2/4 px-8 h-fit">
          <header className="font-light py-3">Thông tin tài khoản</header>
        ...
        <table className="table-auto w-full ">
          ...
          <tbody>
              {order.map((data)=>(
                ...
                <td>{data?.price}</td>
                <td>{data?.quantity}</td>
                <td>{data?.status === 0 ? "chưa xử lý": data?.status === 1 ? "đã giao hàng":"giao hàng thành công"}</td>
                <td><button 
