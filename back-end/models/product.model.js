...
product.updatecart = function(id,quantity,result){
    db.query("UPDATE product SET quantity=quantity - ? WHERE ID=?",[quantity,id],function(err,product){
        if(err){
            result(null);
            return;
        }
...
product.create = function(name,price,quantity,image,result){
    db.query("INSERT INTO product (ID, Name, price, ID_author, ID_publisher, image, status, ID_category, quantity)"
    +"VALUES(NULL,?,?,'1','7',?,'0','1',?)",[name,price,image,quantity],function(err,product){
        if(err){
            result(null);
            return;
        }
        else{
            result({ID: product.insertId, Name:name,price:price,quantity:quantity,image:image});
        }
    });
}
product.delete = function(id,result){
    db.query("UPDATE product SET status='1' WHERE ID=?",id,function(err,product){
        if(err){
            result(null);
            return;
        }else{
...
product.updateproduct = function(id,name,price,quantity,result){
    db.query("UPDATE product SET Name=?,price=?,quantity=quantity+? WHERE ID = ?",[name,price,quantity,id],function(err,product){
        if(err){
            result(null);
            return;
        }
